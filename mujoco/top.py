...
class TOP_Agent:
    ...
    def __init__(
        ...
        Args:
            ...
            beta (float, optional): initial optimism parameter. Defaults to 0.0.
            bandit_lr (float, optional): bandit learning rate. Defaults to 0.1.
            bandit_arms (List, optional): arms for the bandit. If None, uses [-1, 0]. For fixed beta, pass [beta]. Defaults to None.
            learnable_beta (bool, optional): whether to make beta a learnable parameter. Defaults to False.
            beta_lr (float, optional): learning rate for beta if learnable. Defaults to 3e-4.
        """
        self.gamma = gamma
        self.tau = tau
        ...
        # For fixed beta experiments, use single-arm bandit; for standard TOP, use [-1, 0]
        if bandit_arms is None:
            bandit_arms = [-1, 0]
        self.TDC = ExpWeights(arms=bandit_arms, lr=bandit_lr, init=0.0, use_std=True) 
    ...
    def update_policy(self, state_batch: torch.Tensor, beta) -> torch.Tensor:
        ...
        Args:
            state_batch (torch.Tensor): batch of states. 
            beta (float or torch.Tensor): optimism parameter (tensor if learnable for gradient flow).
    ...
    def optimize(
        ...
        Args:
            ...
            beta (float): optimism parameter (used if not learnable).
        ...
        for i in range(n_updates):
            ...
            # Get current beta value (as float for critic - no need to backprop through critics to beta)
