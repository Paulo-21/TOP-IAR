...
def make_gif(policy, env, step_count, state_filter, maxsteps=1000, name=None, out_dir=None):
    envname = env.spec.id
    ...
    #     os.makedirs(gif_dir)
    # clip.write_gif(os.path.join(gif_dir, '{}.gif'.format(gif_name)), fps=30)


def make_checkpoint(agent, step_count: int, env_name: str, out_dir: str = None) -> None:
    ...
    save_path = os.path.join(ckpt_dir, "model-{}-{}.pt".format(step_count, env_name))
