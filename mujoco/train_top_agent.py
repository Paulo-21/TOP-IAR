        ...
        while samples_number < 1e6:
            ...
            while (not done):
                ...
                if cumulative_timestep % gif_interval == 0:
                    ...
                    if save_model:
                        make_checkpoint(agent, cumulative_timestep, params['env'], out_dir=run_dir)
...
def main():
    ...
    parser.set_defaults(fixed_beta=False)
    ...
    # initialize agent
    # For fixed beta baselines, use single-arm bandit; for standard TOP, use default [-1, 0]
    bandit_arms = [params['beta']] if params.get('fixed_beta', False) else None
    agent = TOP_Agent(seed, state_dim, action_dim, \
        ...
        learnable_beta=params['learnable_beta'], beta=params['beta'], \
