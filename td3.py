...
if __name__ == "__main__":
    ...
    with tqdm(total=args.total_timesteps, desc="Training TD3") as pbar:
        for global_step in range(args.total_timesteps):
            ...
            if global_step > args.learning_starts:
                ...
                qf_loss.backward()
                q_optimizer.step()
                # Logging diagnostics similar to TOP-TQC, use TOP tag names (only at log_interval)
                ...
                if global_step % args.policy_frequency == 0:
                    ...
                    for param, target_param in zip(qf2.parameters(), qf2_target.parameters()):
                        target_param.data.copy_(args.tau * param.data + (1 - args.tau) * target_param.data)
                    # Log policy loss (TOP-style, only at log_interval)
                    if writer is not None and args.log_interval and global_step % args.log_interval == 0:
                        try:
